...
def main():
    ...
    # In this scenario we have a single task made of multiple batches.
    # Differently from the New Classes scenario, in NI each batch  may contain
    # patterns of all known classes.
    # For this example we use the MNIST dataset, which is a very popular dataset
    # made of images each containing a single handwritten digit. This means that
    # our dataset contains 10 classes. Here the goal is to improve the
    # classification accuracy of already known classes of digits.
    #
    ...
    # be randomly shuffled with no guarantee about classes distribution in
    # batches. In this last case, you may also find it useful to specify a
    # minimum number of patterns that batches have to contain from avery class
    # by setting the min_class_patterns_in_batch parameter to a value greater
    # than zero.
    ...
    # field of our ni_scenario to get the overall number of classes. This will
    ...
    # forgetting and RAM usage values. You can also keep track of complex
    ...
    # time, as it is very fast. In particular, in NI scenarios Naive is not that
    # bad.
    ...
    for batch_info in ni_scenario:
        ...
        # batch_info the same test set related functions found in the NC
        # scenario. For every training set related function, a test set
        # counterpart exist. However, the methods used to retrieve the current,
        # cumulative, past, future and batch specific test sets will all return
        # the complete test set. That is, they behave like
        # batch_info.complete_test_sets(). These methods were kept with
        # compatibility with the NC counterpart.
        #
