...
def checkpointing_with_plugin(args):
    ...
    train_stream: Sequence[CLExperience] = benchmark.train_stream
    test_stream: Sequence[CLExperience] = benchmark.test_stream
    ...
    # FOURTH CHANGE: USE THE "initial_exp" VARIABLE
    # The train and test loop are coded as usual, but you should use the
    # initial_exp variable to skip the experiences already seen.
    ...
    for train_task in train_stream[initial_exp:]:
        strategy.train(train_task, num_workers=4, persistent_workers=True)
...
def checkpointing_standalone(args):
...
if __name__ == "__main__":
    ...
    checkpointing_standalone(parser.parse_args())
