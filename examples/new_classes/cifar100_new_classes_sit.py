...
# In this example we can see how we can handle a more complex Single Incremental
# Task scenario  with the CIFAR100 dataset. Consider having a look at the
# simpler MNIST example if you haven't already done it yet.
#
# We want to create a scenario in which the first batch contains half of the
# classes. We'll refer to this batch as the "pretrain" batch while next batches
# will be the "incremental" ones.
#
# First, let's define the number of incremental batches in our scenario.
# The first 50 classes will be in the pretrain batch, so this means that we can
# only use 1, 2, 5 or 10, 25 and 50 as the number of incremental batches.

N_INCREMENTAL_BATCHES = 10

# This means that the overall number of batches will be N_INCREMENTAL_BATCHES+1
...
# The crop is followed by a simple normalization, which is also applied to test
# patterns.
...
# arbitrary number of classes per batch. per_batch_classes must be a dictionary
# whose keys are batch IDs and the respective value has to be the number of
# classes to assign to that batch. Other batches will contain an equal number
# of classes among the remaining ones.
