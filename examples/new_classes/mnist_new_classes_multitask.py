...
# Here the goal is to learn to correctly classify new classes of digits without
# forgetting about previously encountered ones.
#
...
# hidden layer.
#
# Our strategy will use a single head approach by saving and loading weights and
# biases depending on the task label. This means that our MLP must have
# "dataset classes"/"tasks" output units, which is the number of classes
# contained in each task. Instead of passing "10/N_TASKS" directly, we can use
# the n_classes and n_tasks fields of our nc_scenario to get the overall number
# of classes and tasks. This will allow us to change MNIST with any another
# dataset with a different number of classes!
...
# in each task. We can use original_classes_in_task to obtain the list of
# classes in each task. You can obtain a different class order by changing the
# seed passed to create_nc_single_dataset_multi_task_scenario.
#
# Beware that "original"_classes_in_task will contain the class IDs from
# MNIST dataset. Those IDs will be different from the ones used while trainig
# and testing our Continual Learning strategy: we use an head for each task
# which means that, for each task, class IDs must always be in range
# [0, output_units). """Luckly""", nc_scenario already takes care of this.
# With this in mind, note the difference between the contents
# original_classes_in_task and classes_in_task.
print('Task order:')
for task_idx in range(nc_scenario.n_tasks):
    print('Task', task_idx)
    print('\toriginal classes:', nc_scenario.original_classes_in_task[task_idx])
    print('\tmapped classes:', nc_scenario.classes_in_task[task_idx])
...
# This instance exposes a lot of useful information about the current tasl
...
for task_info in nc_scenario:
    ...
    # Note: you can also keep track of the current task ID by using enumerate
    # in the for loop!
    ...
    # task_info exposes useful test set related functions, too. As with their
    ...
    # current or a specific task test set will just return a tuple with the
    # same format.
    #
    ...
    # (Dataset, task_label). """Fortunately""", task_info.complete_test_sets()
    ...
    # you'll have to wrap the tuple returned by task_info.current_test_set() or
    # task_info.task_specific_test_set(x) inside a list by creating a
