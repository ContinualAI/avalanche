...
class MetricCollectorTests(unittest.TestCase):
    def test_time_reduce(self):
        ...
        v = mc.get("FakeMetric", time_reduce=None, exp_reduce="sample_mean", stream=fake_stream)
        np.testing.assert_array_almost_equal(v, [(1+3*2)/3, (5+7*2)/3, (11+13*2)/3])
        v = mc.get("FakeMetric", time_reduce=None, exp_reduce="experience_mean", stream=fake_stream)
        np.testing.assert_array_almost_equal(v, [(1+3)/2, (5+7)/2, (11+13)/2])

        # time = "last"
        v = mc.get("FakeMetric", time_reduce="last", exp_reduce=None, stream=fake_stream)
        np.testing.assert_array_almost_equal(v, [11, 13])
        v = mc.get("FakeMetric", time_reduce="last", exp_reduce="sample_mean", stream=fake_stream)
        self.assertAlmostEqual(v, (11+13*2)/3)
        v = mc.get("FakeMetric", time_reduce="last", exp_reduce="experience_mean", stream=fake_stream)
        self.assertAlmostEqual(v, (11+13)/2)

        # time_reduce = "mean"
        v = mc.get("FakeMetric", time_reduce="mean", exp_reduce=None, stream=fake_stream)
        np.testing.assert_array_almost_equal(v, [(1+5+11)/3, (3+7+13)/3])
        v = mc.get("FakeMetric", time_reduce="mean", exp_reduce="sample_mean", stream=fake_stream)
        self.assertAlmostEqual(v, ((1+5+11)/3 + (3+7+13)/3*2) / 3)
        v = mc.get("FakeMetric", time_reduce="mean", exp_reduce="experience_mean", stream=fake_stream)
