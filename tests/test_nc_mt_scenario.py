...
class MultiTaskTests(unittest.TestCase):
    def test_mt_single_dataset(self):
        ...
        nc_scenario = NCBenchmark(
            ...
            seed=1234, class_ids_from_zero_in_each_step=True)

        self.assertEqual(5, nc_scenario.n_steps)
        self.assertEqual(10, nc_scenario.n_classes)
        for task_id in range(5):
            self.assertEqual(2, len(nc_scenario.classes_in_step[task_id]))

        all_classes = set()
        all_original_classes = set()
        for task_id in range(5):
            all_classes.update(nc_scenario.classes_in_step[task_id])
            all_original_classes.update(
                nc_scenario.original_classes_in_step[task_id])
    ...
    def test_mt_single_dataset_without_class_id_remap(self):
        ...
        nc_scenario = NCBenchmark(
            mnist_train, mnist_test, 5, task_labels=True, shuffle=True,
            seed=1234, class_ids_from_zero_in_each_step=False)
    ...
    def test_mt_single_dataset_fixed_order(self):
        ...
        all_classes = []
        for task_id in range(5):
            all_classes.extend(nc_scenario.classes_in_step[task_id])
    ...
    def test_sit_single_dataset_fixed_order_subset(self):
        ...
        nc_scenario = NCBenchmark(
            ...
            fixed_class_order=order, class_ids_from_zero_in_each_step=True)
    ...
    def test_sit_single_dataset_fixed_subset_no_remap_idx(self):
        ...
        self.assertEqual(2, len(nc_scenario.classes_in_step))

        all_classes = set()
        for task_id in range(2):
            self.assertEqual(4, len(nc_scenario.classes_in_step[task_id]))
    ...
    def test_mt_multi_dataset_one_task_per_set(self):
        ...
        nc_scenario = NCBenchmark(
            ...
            class_ids_from_zero_in_each_step=True, one_dataset_per_step=True)
        ...
        self.assertTrue(
            (nc_scenario.classes_in_step[0] == {0, 1, 2} and
             nc_scenario.classes_in_step[1] == set(range(0, 7))) or
            (nc_scenario.classes_in_step[0] == set(range(0, 7)) and
