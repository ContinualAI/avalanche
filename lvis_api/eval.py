...
class LVISEval:
    def __init__(self, lvis_gt, lvis_dt, iou_type="segm"):
        ...
        if isinstance(lvis_dt, LVIS):
            self.lvis_dt = lvis_dt
        elif isinstance(lvis_dt, (str, list)):
            self.lvis_dt = LVIS(self.lvis_gt, lvis_dt)
        else:
    ...
    def evaluate(self):
        ...
        self.logger.debug("Starting _prepare")
        start_time = time.time()
        self._prepare()
        self.logger.debug("_prepare completed in %.4f", time.time() - start_time)

        self.logger.debug("Starting overall compute_iou")
        start_time = time.time()
        self.ious = {
        ...
        self.logger.debug("overall compute_iou completed in %.4f",
                          time.time() - start_time)

        # loop through images, area range, max detection number
        self.logger.debug("Starting overall evaluate_img")
        start_time = time.time()
        self.eval_imgs = [
        ...
        self.logger.debug("overall evaluate_img completed in %.4f",
                          time.time() - start_time)
