...
Each strategy object offers two main methods `train` and `test`. Both of them, accept either a _single step_ \(`IExperience`\) or a _list of them_, for maximum flexibility.
...
scenario = nc_scenario(
    mnist_train, mnist_test, n_steps, shuffle=True, seed=1234,
    task_labels=False)
# ---------
...
In Avalanche this can be done with another basic strategy you can extend called `JointTraining`. `JointTraining` follows roughtly the same ideas and API of the `BaseStrategy` but instead of processing steps in a streams iteratively it collapse them into a single, big dataset from which to learn.
