...
* **Strategies**: it contains the collection of pre-implemented baselines you can use for comparisons.
...
* **Plugins**: plugins are modules implementing some specific \(and often reusable across strategies\) behaviours you can attach to your own strategy.
...
In general, we suggest to work with the _Intermediate Mode_, which offers in our opinion the right level of automation at a reasonable complexity, even for _Avalanche_ "_newbies_". But let's see an example for each of those modalities!
...
Then, we can use our strategy as we would do for the pre-implemented ones. However, as previously hinted, some evaluation utilities will be not included automatically and should be called by hand. Please refer to the follow-up notebook for more details about what the `evaluation` module can offer.
...
So the added modularity helps us **reducing overhead and code duplication**, **improving code readability and prototipying speed**.
...
As you could have noticed from the previous examples, parameters are automatically passed as inputs using the decorators _TrainingFlow_ and _TestingFlow_. The flow will inject the correct parameter values by looking at different locations:
...
At any moment you can find out all the parameters you can use as follows:
...
For convenience, we also report in the table below the signature of each methods and the additional parameters you can get from the namespace, at least in the basic version of the `MTDeepLearningStrategy` class you can inherit and fully customize.
...
If the standard training and testing flows, for some reason, do not fit in your ideal continual learning strategy, you're free to change them as well. You can add new callbacks \(inject a new method into the existing flow\) or even change the order of the existing one. Everything is possibile in _Avalanche_! :-\)
...
The basic idea is rather to **attach them to the main strategy so that they can augment its behaviors.** In practice this means that in the flow execution, for every method of the main strategy, the pugin\(s\) corresponding methods **will be called first** \(if implemented\).
...
In general we recommend to implement a Strategy via pugins, if possible. This will allow other people to use them and facilitate interoperability among different approaches.

For example, we could have implemented a replay strategy as a standalone `MTDeepLearningStrategy` object. However, creating a plugin to be used in conjunction with the `Naive` strategy is much better, since _experience replay_ is part of many different continual learning strategies:
