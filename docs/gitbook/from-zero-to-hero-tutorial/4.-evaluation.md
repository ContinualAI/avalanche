...
* **Metrics**: a set of classes \(one for metric\) which implement the main continual learning metrics, like _accuracy_, _forgetting_, _memory usage_, _running times_, etc.
...
* **Evaluation Protocols**: this module provides a single point of entry to the evaluation methodology configuration and in charge of handling the metrics computation, tensorboard or avanced console logging.
...
At the moment we implemented just the `TensorboardLogging` object that can be used to specify Tensorboard behaviors with eventual customizations.
