...
The `bechmarks` module offers 3 types of utilities:
...
Each of these `streams` are _iterable_, _indexable_ and _sliceable_ objects that are composed of unique **steps**. Steps are batch of data \(or "_tasks_"\) that can be provided with or without a specific task label.
...
# The training and test set used to generate the incremental steps.
...
# train_dataset field.
perm_mnist.train_steps_patterns_assignment

# A list containing which test patterns are assigned to each step.
# Patterns are identified by their id w.r.t. the dataset found in the 
# test_dataset field
perm_mnist.test_steps_patterns_assignment
...
Each stream can in turn be treated as an iterator that produces a unique `step`, containing all the useful data regarding a _batch_ or _task_ in the continual stream our algorithms will face. Check out how can you use these steps below:
...
t_label = step.task_label
dataset = step.dataset

# but you can recover additional info
step.current_step
step.classes_in_this_step
step.classes_seen_so_far
step.previous_classes
step.future_classes
step.origin_stream
step.scenario
...
## 🏛️ Classic Benchmarks
 ...
 Now that we know how our benchmarks work in general through scenarios, streams and steps objects, in this section we are going to explore **common benchmarks** already available for you with one line of code yet flexible anough to allow proper tuning based on your needs:
...
for step in train_stream:
    ...
    training_dataset = step.dataset
    print('Task {} batch {} -> train'.format(t, step_id))
    print('This batch contains', len(training_dataset), 'patterns')
```

 Or, we can create a benchamark based on MNIST and the _Class-Incremental_ \(what's commonly referred to as "_Split-MNIST_" benchmark\):
