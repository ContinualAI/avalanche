...
class CumulativePlugin(StrategySkeleton):
    """
    A Cumulative CL Plugin in which, at each step (or task), the model
    is trained with all the data encountered so far. Therefore, at each step,
    the model is trained in a MultiTask scenario.
    The resulting strategy has a high memory and computational cost.
    """

    def __init__(self):
        super(CumulativePlugin, self).__init__()
