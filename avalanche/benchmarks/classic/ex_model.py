...
from torchvision.transforms import RandomHorizontalFlip, RandomCrop, \
    RandomRotation, ToTensor, CenterCrop, Normalize, Resize
...
def _load_expert_models(scenario_name, run_id, len_stream):
    ...
    weburl = f'http://131.114.50.174/data/EXML_CLVISION22_PRETRAINED_EXPERTS' \
...
def check_experts_accuracy(exml_benchmark):
    ...
    print(type(exml_benchmark).__name__,
    ...
    for i, model in enumerate(exml_benchmark.expert_models_stream):
        ...
        for x, y, t in DataLoader(train_data, batch_size=256,
...
class ExMLCoRE50(ExModelCLScenario):
    ...
    def __init__(self, scenario='ni', run_id=0):
        ...
        assert scenario in {'ni', 'joint', 'nc'}, \
        ...
        if scenario == 'ni':
            ...
            benchmark = nc_benchmark(train_cat, test_cat, n_experiences=1,
                                    task_labels=False)
        else:
            assert False, "Should never get here."
...
class ExMLCIFAR10(ExModelCLScenario):
    ...
    def __init__(self, scenario='split', run_id=0):
        ...
        if scenario == 'split':
            ...
            benchmark = SplitCIFAR10(n_experiences=1, return_task_id=False,
                                    seed=CURR_SEED)
        else:
            assert False, "Should never get here."
